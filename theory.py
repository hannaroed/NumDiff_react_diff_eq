...
def exact_solution_test_case(x: np.ndarray, t: np.ndarray, mu: float, a: float, L: float) -> np.ndarray:
    ...
    This version supports both scalar and array inputs for `t`, allowing for efficient computation across multiple time steps.
    """
...
for i in trange(len(Nx_values)):
    ...
    error = np.max(np.abs(u_num1 - u_exact))
    #print(f"Nx = {Nx}, Error = {error}")
